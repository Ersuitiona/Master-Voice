...
export const DLSTrainerMode: React.FC<Props> = ({ scenarios, onStartCall }) => {
  ...
  return (
    <div className="space-y-8 pb-16">
      {/* Header Banner */}
      <div className="rounded-2xl bg-gradient-to-r from-[#16161D] via-[#1A1A24] to-[#12121A] border border-indigo-500/20 p-8 shadow-2xl relative overflow-hidden">
        <div className="space-y-3 relative z-10 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-indigo-500/10 text-indigo-300 text-xs font-bold border border-indigo-500/20 uppercase tracking-wider">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredScenarios.map((s) => (
          <div
            key={s.id}
            className="p-6 rounded-2xl bg-gradient-to-b from-[#16161D] to-[#0E0E12] border border-white/10 hover:border-indigo-500/40 transition-all flex flex-col justify-between space-y-4"
          >
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 text-xs font-bold uppercase">
                  {s.category}
                </span>
              ...
              <div className="bg-black/40 p-3.5 rounded-xl border border-white/5 text-xs space-y-1">
                <div className="flex justify-between">
                  <span className="text-slate-400">Caller / Employee:</span>
                  <span className="font-semibold text-slate-200">{s.customerName}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Persona & Tone:</span>
                  <span className="font-semibold text-indigo-400">{s.personality}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Request ID:</span>
                  <span className="font-mono text-slate-300">{s.caseId}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                ...
                className="flex-1 py-3 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/30 transition-all hover:scale-105"
              >
